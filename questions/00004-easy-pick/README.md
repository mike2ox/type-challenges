...
Implement the built-in `Pick<T, K>` generic without using it. (clear)
...
1. extends keyof T로 확장 => K에서 T의 key는 갖고있음이 보장해주기 위해 설정
2. key in T -> 실제 key로 쓸 것들은 K가 아니라 T에만 있는 것들로 구성
```
